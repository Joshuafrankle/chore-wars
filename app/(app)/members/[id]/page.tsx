...
export default async function MemberProfilePage({
  ...
  const [{ data: member }, { data: completions }, { data: allCompletions }, { data: members }] =
    await Promise.all([
      ...
      // Also used for the streak below — capped at 20, so an active streak
      // longer than that would be undercounted (acceptable for now; the
      // history list needs a cap regardless).
      supabase
        .from("chore_completions")
        .select("id, completed_at, effort_awarded, chores(name), chore_assignments(due_date)")
        .eq("household_id", householdId)
        .eq("user_id", id)
  ...
  const streak = computeStreak(
    (completions ?? [])
      .filter((completion) => completion.chore_assignments)
      .map((completion) => ({
